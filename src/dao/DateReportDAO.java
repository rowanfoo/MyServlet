...
public class DateReportDAO {
	...
	public ArrayList <DataReport>   getTenMonthAvgHistory (String code)throws Exception{
		...
		String mysql="SELECT date, min(twohundredchg) as low , max(twohundredchg) as high  FROM fortune.data where code=? and year(date)>=? group by month(date) , Year(date) order by (date)";
	...
	public ArrayList <DataReport>   getFiftyDayTwoYears (String code)throws Exception{
		...
		String mysql="SELECT   date,code,   format((fiftychg*100),2)    as fiftychg FROM fortune.data where Year(date)=? and  code=? order by date desc";
	...
	return arr;
		...
		public ArrayList <DataReport>   getMonthAverageFromFiftyDayDownFourYears (String code)throws Exception{
			...
			//String mysql="SELECT   date,code,   format((avg(fiftychg )*100),2)    as fiftychg FROM fortune.data where Year(date)=? and  code=? and  fiftychg<=0 group by Month(date)order by date desc";
			String mysql="SELECT   date,code,   format((avg(fiftychg )*100),2)    as fiftychg FROM fortune.data where Year(date)=? and  code=?  group by Month(date)order by date desc";
		...
		public ArrayList <DataReport>   getDownvsUpDayRatio (String code)throws Exception{
			...
			String mysql="SELECT  date,count(*)as countdown FROM fortune.data where Year(date)=? and  code=? and  changePercent < 0 group by Month(date) order by date desc";
			String mysql2="SELECT  date,count(*)as countup FROM fortune.data where Year(date)=? and  code=? and  changePercent > 0 group by Month(date) order by date desc";
		...
		public ArrayList <DataReport>   getDownvsUpVolumeRatio (String code)throws Exception{
			...
			String mysql="SELECT  date,sum(volume) as downvol FROM fortune.data where Year(date)=? and  code=? and  changePercent < 0 group by Month(date) order by date desc";
			String mysql2="SELECT  date,sum(volume) as upvol FROM fortune.data where Year(date)=? and  code=? and  changePercent > 0 group by Month(date) order by date desc";
...
/*
	...
	*/
	...
	public ArrayList <DataReport>   getPriceVariance (String code)throws Exception{
		...
		String mysql="select code ,date, VARIANCE(changePercent)as var from data where month(date)=? and code=?  and year(date)=? group by Month(date) ";

...
public class MovingAverageDAO extends DAO{
	...
	public MovingAverageAccess getStockMovingAverage(String code)throws Exception{
		String mysql="select * from (SELECT count(*)as nodays FROM data where date >= DAYSADDNOWK(current_date(),60) and code=? and twentychg < 0)a,"+
					"(SELECT count(*)as nodays1 FROM data where date >= DAYSADDNOWK(current_date(),120) and code=? and fiftychg < 0)b,"+
					"(SELECT count(*)as nodays2 FROM data where date >= DAYSADDNOWK(current_date(),240) and code=? and fiftychg < 0)c";
